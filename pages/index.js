...
const LoginForm = ({ setError, setToken }) =>{
  ...
  const [ validateOtp, { validateLoading, validateError } ] = useMutation(VALIDATE_OTP, {
    onError: (error) => {
        setFormLoader(false)
        addToast(error.graphQLErrors[0].message, {
            appearance: 'warning',
    ...
    onCompleted({validateOtp}){
        ...
        updatePasswordRequest({ variables: { id: validateOtp.id, password, old_password: confirm_password } })
        setMode(2)
    }
  ...
  //update password
  const [ updatePasswordRequest, { updatePasswordLoading, updatePasswordError } ] = useMutation(UPDATE_PASSWORD, {
    onError: (error) => {
        setFormLoader(false)
        addToast(error.graphQLErrors[0].message, {
            appearance: 'warning',
  ...
  return (
    ...
    <div className="container-fluid px-0">
        ...
        <section className="section-one">
            <header className="page-header row">
                <div className="col-lg-7 d-flex flex-column justify-content-center">
                    ...
                    <p className="page-para">a fast growing  financial society in Nigeria . we are interested in delivering excellent, quality and speedy services for customers satisfaction while consistently being transparent, expanding our clients base and legacy nationally and internationally.</p>
                ...
                { mode === 0 &&
                    <form className="form" onSubmit={submit}>
                        ...
                        <p><a href="#" className="forgot-pw" onClick={() => setMode(1)}>Forgot password?</a></p>
                ...
                { mode === 1 &&
                    <form className="form" onSubmit={sendOtp}>
                        ...
                        <p><a href="#" className="forgot-pw" onClick={() => setMode(0)}>Login</a></p>
                        {isError && <p className="error page-para">{errorMessage}</p>}
                        {isSuccess && <p className="success page-para">{successMessage}</p>}
                        <button disabled={loading} type="submit" className="btn btn-lg btn-block btn-primary" style={{fontSize: "16px", color: "#fff"}}>
                          {
                ...
                { mode === 2 &&
                    <form className="form" onSubmit={submitValidatOtp}>
                        ...
                        <div className="form-group">
                            ...
                            onChange={({ target }) => {checkPasswordMatch(target.value)}}
                             placeholder="******" />
                            {ispasswordMatch && <p className="error page-para">Password do not match</p>}
                        </div>
                        <p><a href="#" className="forgot-pw" onClick={() => setMode(0)}>Login</a></p>
                        {isError && <p className="error page-para">{errorMessage}</p>}
                        <button disabled={loading} type="submit" className="btn btn-lg btn-block btn-primary" style={{fontSize: "16px", color: "#fff"}}>
        ...
        <footer className="page-footer py-5">
            <p className="footer-para mb-0"> Copyright © {currentYear} <span style={{color: "#0081FF"}} className="px-1">Kasu Cooperative.
